...
export class ProviderFactory {
  ...
  static create(config: AutohandConfig): LLMProvider {
    ...
    switch (providerName) {
      ...
      case "github-copilot":
        if (!config["github-copilot"]) {
          return new UnconfiguredProvider("github-copilot");
        }
        return new GitHubCopilotProvider(config["github-copilot"]);
   ...
   * Note: This checks if the name is a valid provider type, not if it is available on this platform.
